...
function view(){
	...
	for (let i = 0; i < this.layerCount; i++) {
		results.push({
			weights: this.layerLink[i].getWeights().data,
			bias: this.layerLink[i].getBias().data
		});
	}
	return results;
}
...
export class NeuralNetwork {
	...
	init(layers){
		...
		for (let i = 1, j = 0; j < (this.layerCount); i++, j++) {
			...
			// Previous Layer Nodes & Current Layer Nodes
			this.layerLink[j] = new LayerLink(layers[j], layers[i]);
		}
	}
	...
	setOptions(options){
		...
		if (options) {
			...
			if (options.activation && options.derivative) {
				if (options.activation instanceof Function &&
